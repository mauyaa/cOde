          ...
          {orders.map((o) => {
            ...
            return (
              ...
              >
                ...
                <Box>
                  <Typography variant="body1" sx={{ fontWeight: 500, color: 'text.primary', fontSize: type.sm }}>
                    {o.productName || '—'}
                  </Typography>
                  <Typography variant="body2" sx={{ fontSize: type.xs, color: colors.ghost }}>
                    {o.buyerName || o.sellerName || '—'}
                    {o.quantity ? ` · ${o.quantity}${o.productUnit || ' units'}` : ''}
                  </Typography>
                </Box>
                <Box sx={{ textAlign: 'right' }}>
                  <Typography variant="body1" sx={{ fontWeight: 500, fontSize: type.sm, color: 'text.primary' }}>
                ...
                <Box sx={{ display: 'flex', gap: 0.5, justifyContent: 'flex-end' }}>
                  ...
                  {showComplete && (
                    <button onClick={() => handleStatusUpdate(o.id, 'completed')}
                      style={{ border: `1px solid ${colors.green[300]}`, borderRadius: 4, background: colors.green[500], color: colors.surface,
                               cursor: 'pointer', padding: '4px 10px', fontFamily: 'inherit', fontSize: type.xs, fontWeight: 500 }}>
                      Done
                    </button>
                  )}
                  {showCancel && (
                    <button onClick={() => handleStatusUpdate(o.id, 'cancelled')}
                      style={{ border: `1px solid ${colors.horizon}`, borderRadius: 4, background: 'transparent', color: colors.muted,
      ...
      <Box sx={{ display: 'flex', height: 500, gap: 3 }}>
        ...
        <Box
          ...
          sx={{
            ...
            borderRight: `1px solid ${colors.horizon}`,
            overflowY: 'auto',
          }}
        >
          {conversations.length === 0 ? (
            <EmptyState title="No conversations"
              description="Messages from buyers to sellers will appear here."
            />
          ) : (
            conversations.map((c, i) => {
              const last = c.lastMessage || c.messages?.[c.messages.length - 1] || {};
              const active = detailConv?.id === c.id;
              return (
                <button
                  key={c.id || i}
                  role="listitem"
                  onClick={() => { setDetailConv(c); setMessages([]); }}
                  style={{
                    width: '100%', textAlign: 'left', border: 'none', background: active ? colors.elevated : 'transparent',
                    cursor: 'pointer', padding: '14px 14px 14px 12px',
                    borderBottom: `1px solid ${colors.horizon}`,
                    display: 'flex', flexDirection: 'column', gap: 3, fontFamily: 'inherit',
                  }}
                >
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Typography variant="body2" sx={{ fontWeight: 500, fontSize: type.sm, color: 'text.primary' }}>
                      {c.title || c.sellerName || 'Conversation'}
                    </Typography>
                    <Typography variant="caption" sx={{ color: colors.ghost, fontSize: type.xs, whiteSpace: 'nowrap' }}>
